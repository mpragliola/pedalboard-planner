...
| **Pointer down** on catalog item (board or device) | CatalogList / CatalogListGrouped item button | Start catalog item session: long-press timer + window pointer move/up/cancel. Tap → add at viewport center; move beyond threshold → cancel (scroll); long-press → start catalog drag (ghost + panel slides away) |
| **Pointer move** (during catalog item session)     | window (capture)                             | Before long-press: if movement &gt; threshold, cancel (scroll). After long-press: update ghost position                                                                                                          |
| **Pointer up / pointer cancel** (catalog drag)     | window (capture)                             | End catalog drag: remove body class (panel back), place object on canvas if drop point inside canvas, else no place                                                                                              |
