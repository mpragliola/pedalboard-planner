...
export function useBoardDeviceFilters() {
  ...
  const deviceBrands = useMemo(() => {
    const set = new Set(DEVICE_TEMPLATES.map((t) => t.brand).filter(Boolean))
    return Array.from(set).sort((a, b) => a.localeCompare(b))
  }, [])
  ...
  }, [deviceBrandFilter, deviceTypeFilter, deviceTextFilter, deviceWidthMin, deviceWidthMax, deviceDepthMin, deviceDepthMax])
